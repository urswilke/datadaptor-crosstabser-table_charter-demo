...
#| input:
#|   - Free1
Free1 <- Free1 |> dplyr::mutate(across(
  everything(), 
...
Here we'll show some examples how you can generate tables along the y-axis of the crosstabs[^3]:
...
inputData = [
  { Abbreviation: "age", Title: "Age", Type: "cat", RowVar: "age", MetrMac: "S1M0" },
  { Abbreviation: "fav_it", Title: "What`s your favorite Italian food?", Type: "mdg", RowVar: "q1_1 q1_2 q1_3", MdgMissLab: "No answer", MetrMac: "" },
  { Abbreviation: "fav_de", Title: "What`s your favorite German food?", Type: "mcg", RowVar: "q2n1 q2n2 q2n3", MetrMac: "" },
  { Abbreviation: "item_bat", Title: "How much do you agree?", Type: "mw", RowVar: "q3_1 q3_2", "Freq": "0", MetrMac: "" },
]
```
...
* execute `Mapping$modify_data()` (as we pass `df` as the dat argument; see [here](https://urswilke.gitlab.io/crosstabser/reference/Tabula.html#method-new-)), and
* call `Tabula$calc_qtabs()`.

The `print()` call then reprints[^4] the calculated crosstabs, 
and finally we recalculate the data input for the table_charter app in @sec-tab-ch:

[^4]: Unlike this wasm version in your browser, running the crosstabser R package in RStudio will give you a color-formatted print output. The formatting also works in the R console on Linux, but not on Windows.
...
and then adding a `<table-charter>` html tag with the crosstab data from R as its data attribute[^5]:

[^5]: Here we use [observable's hypertext literal](https://observablehq.com/@observablehq/htl) to generate the html in a quarto ojs cell.
